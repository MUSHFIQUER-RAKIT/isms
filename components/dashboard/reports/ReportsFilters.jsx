...
export default function ReportsFilters({ customer, createdBy, children }) {
  ...
  const reportSort = searchParams.get("report");
        ...
        <div className="grid grid-cols-1 h- lg:grid-cols-4 gap-8">
          ...
          <aside className="hidden bg-[var(--color-card)] border border-[var(--color-border)] rounded-xl p-5 shadow-md lg:block space-y-6">
            ...
            {filters.map((section) => (
              <div
                key={section.id}
                className="border-b flex flex-col text-right items-end gap-3 pb-4"
              >
                <button
                  onClick={() =>
                    setOpenSection(
                ...
                {openSection === section.id && (
                  <div className="mt-3 flex flex-wrap gap-2 text-right space-y-2">
                    {section.options.map((opt) => (
                      <label
                        key={opt.id}
                        className="flex gap-1 bg-accent/10 cursor-pointer rounded p-2"
                      >
                        {opt.name}
                        <input
                          type="checkbox"
                          onChange={() =>
                            handleFilter(section.name.toLowerCase(), opt.id)
                          }
                          className="rounded accent-accent border-gray-400"
                        />
                      </label>
                    ))}
                  </div>
